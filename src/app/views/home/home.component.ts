import { NgClass, NgStyle } from '@angular/common';
import { Component } from '@angular/core';

@Component({
  selector: 'app-home',
  standalone: true,
  imports: [NgStyle, NgClass],
  templateUrl: './home.component.html',
  styleUrl: './home.component.css'
})
export class HomeComponent {
  //Declaro una variable con las imágenes y los textos que tengo que mostrar
  public firstPhotos: { img1: string; img2: string; text: string }[] = [
    {
      img1: 'https://cdn.vox-cdn.com/thumbor/qvJ_PVw2Ppnemy8hxFU2U3RojK0=/1400x0/filters:no_upscale()/cdn.vox-cdn.com/uploads/chorus_asset/file/23165626/memory2final.jpg',
      img2: 'https://conductscience.com/maze/wp-content/uploads/2019/10/Diseases-and-conditions-that-affect-short-term-memory.png',
      text: 'Memoria corto plazo'
    },
    {
      img1: 'https://cdn.psychologytoday.com/sites/default/files/styles/article-inline-half/public/field_blog_entry_images/1369847707_4085_memory-1.jpg?itok=aZRKeJXR',
      img2: 'https://conductscience.com/maze/wp-content/uploads/2019/12/Diseases-and-conditions-that-affect-long-term-memory.png',
      text: 'Memoria largo plazo'
    },
    {
      img1: 'data:image/jpeg;base64,',
      img2: 'https://media.springernature.com/m685/springer-static/image/art%3A10.1038%2Fnrn2850/MediaObjects/41583_2010_Article_BFnrn2850_Fig1_HTML.jpg',
      text: 'Memoria declarativa'
    },
    {
      img1: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTF8a2YKUyrUrEEkYxECnP4zI3LXpfEhhEZhg&usqp=CAU',
      img2: 'https://static.wixstatic.com/media/a7b893_49ac93653aea4e3da7f46b5de20e8ede~mv2.png/v1/fill/w_640,h_640,al_c,q_90,usm_0.66_1.00_0.01,enc_auto/a7b893_49ac93653aea4e3da7f46b5de20e8ede~mv2.png',
      text: 'Memoria procedimental'
    },
    {
      img1: 'https://easyscienceforkids.com/wp-content/uploads/2018/04/Memory-facts-18-4-1-758x635.jpg',
      img2: 'https://i.pinimg.com/736x/66/18/74/661874b7751be9d8421e955e2af4213a.jpg',
      text: 'Memoria sensorial'
    },
    {
      img1: 'https://insidememory721788887.files.wordpress.com/2018/10/memory.jpg',
      img2: 'https://www.wikihow.com/images/thumb/7/75/Improve-Verbal-Memory-Step-5-Version-2.jpg/v4-460px-Improve-Verbal-Memory-Step-5-Version-2.jpg',
      text: 'Memoria verbal'
    }
  ];

  //Declaro las variables booleanas que necesito
  public firstPhoto: boolean = true;
  public memoria: boolean = true;
  public types: boolean = false;
  public tips: boolean = false;
  //Declara las variables vacías para cambiar las imágenes
  public oneImage: string = '';
  public twoImage: string = '';
  //declaro la variable para el texto inicial
  public text: string = 'Memoria';
  //Declaro la variable para el case al iniciar la página
  public case: number = 0;
  //declaro la clase que cambiará según la imagen
  public classImage: string = 'image-container2';

  //Método para ampliar la imagen seleccionada y cambiar el texto a mostrar
  public onClick(index: number): void {
    this.oneImage = this.firstPhotos[index].img1;
    this.twoImage = this.firstPhotos[index].img2;
    this.classImage = 'image-container';
    this.text = this.firstPhotos[index].text;
    this.memoria = false;
    this.types = true;
    this.case = index;
  }

  //Método para cambiar a la imagen de consejos y textos
  public onClickImage(): void {
    this.types = false;
    this.firstPhoto = false;
    this.tips = true;
  }

  //Método para volver a las imágenes iniciales
  public onClickImageTips(): void {
    this.types = false;
    this.firstPhoto = true;
    this.memoria = true;
    this.classImage = 'image-container2';
    this.text = 'Memoria';
  }
}

